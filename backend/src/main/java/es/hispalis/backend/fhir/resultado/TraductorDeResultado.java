...
public class TraductorDeResultado {
    ...
    /** El catálogo local del laboratorio; la traducción a LOINC la hace el `ConceptMap` de la guía. */
    private static final String CATALOGO_PRUEBAS =
            "https://aojeda006.github.io/HispaLIS/fhir/CodeSystem/catalogo-pruebas";
     ...
     */
    public Resultado aDominio(Observation recurso, Especimen especimen) {
        ...
        if (recurso.hasValueStringType()) {
            return Resultado.informarTextual(
                    especimen, codigo, recurso.getValueStringType().getValue());
        }
    ...
    public Observation aFhir(Resultado resultado) {
        ...
        recurso.setCode(new CodeableConcept()
                .addCoding(new Coding().setSystem(CATALOGO_PRUEBAS).setCode(resultado.codigoDePrueba())));
    ...
    private static String codigoDelCatalogo(Observation recurso) {
        return recurso.getCode().getCoding().stream()
                .filter(codigo -> CATALOGO_PRUEBAS.equals(codigo.getSystem()))
                .map(Coding::getCode)
                .findFirst()
